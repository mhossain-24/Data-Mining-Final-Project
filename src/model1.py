...
Original file is located at
    https://colab.research.google.com/drive/1r2oLlUUB6ezSFeX1naasQWow5ep4uqQi
"""
...
RARE_THRESH = 200  # increase only classes below this instance count
...
def choose_heads(c: int, max_heads: int = 8) -> int:
    for h in range(min(max_heads, c), 0, -1):
        if c % h == 0:
            return h
    return 1

class TransformerAttention2D(nn.Module):
    def __init__(self, c: int, heads=None, mlp_ratio: int = 4, dropout: float = 0.0):
        super().__init__()
        self.c = c
        self.heads = heads if heads is not None else choose_heads(c)
        self.ln1 = nn.LayerNorm(c)
        self.attn = nn.MultiheadAttention(embed_dim=c, num_heads=self.heads, batch_first=True, dropout=dropout)
        self.ln2 = nn.LayerNorm(c)
        hidden = mlp_ratio * c
        self.mlp = nn.Sequential(
            nn.Linear(c, hidden), nn.GELU(), nn.Dropout(dropout),
            nn.Linear(hidden, c), nn.Dropout(dropout),
        )
    def forward(self, x):
        b, c, h, w = x.shape
        ...
        y2 = self.ln2(tokens)
        tokens = tokens + self.mlp(y2)
        return tokens.transpose(1, 2).reshape(b, c, h, w)

class WrapPreDetectWithP5Attention(nn.Module):
    def __init__(self, base: nn.Module, attn: nn.Module):
        super().__init__()
        self.base = base
        self.attn = attn
...
def insert_p5_attention(det_model, detect_idx, imgsz=640):
    C = probe_p5_channels(det_model, detect_idx, imgsz)
    heads = choose_heads(C, max_heads=8)
    pre = det_model.model[detect_idx - 1]
    attn = TransformerAttention2D(c=C, heads=heads, mlp_ratio=4, dropout=0.0).to(device)
    det_model.model[detect_idx - 1] = WrapPreDetectWithP5Attention(pre, attn)
    return C

def ssl_pretrain_yolo11s_attn(img_dir_for_ssl, imgsz=640, ssl_epochs=8, ssl_batch=32, ssl_lr=3e-4, temp=0.2,
                              save_pt="/kaggle/working/yolo11s_ssl_attn.pt"):
    y = YOLO("yolo11s.pt")
    ...
    C = insert_p5_attention(det_model, detect_idx, imgsz=imgsz)
    ...
    for ep in range(ssl_epochs):
        ...
        print(f"[SSL+ATTN] ep {ep+1}/{ssl_epochs} loss {total/len(dl):.4f}")

    hh.remove()
    y.save(save_pt)
    print("Saved checkpoint:", save_pt)
    return save_pt

SSL_ATTN_PT = ssl_pretrain_yolo11s_attn(
...
run_name = f"D_ssl_attention_lr{TRAIN_LR0}_b{TRAIN_BATCH}"
y = YOLO(SSL_ATTN_PT)
...
pred_out = "/kaggle/output/test_predictions_D"
save_test_prediction_images(ckpt, test_img_dir, pred_out, imgsz=IMG_SIZE, conf=0.25, n=16)

print("VAL:", val_met)
print("TEST:", test_met)

export_variant("D_ssl_attention", run_name, run_dir, ckpt, val_met, test_met)